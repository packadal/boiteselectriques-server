...
class OscReceiver {
 public:
  using message_handler =
      std::function<void(osc::ReceivedMessageArgumentStream)>;
  using connection_handler =
      std::function<void(osc::ReceivedMessageArgumentStream, std::string)>;

  OscReceiver(unsigned int port) { setPort(port); }
    ...
    while (!ok) {
      try {
        socket = std::make_shared<UdpListeningReceiveSocket>(
            IpEndpointName(IpEndpointName::ANY_ADDRESS, _port), &_impl);
        ok = true;
 ...
 private:
  ...
  class : public osc::OscPacketListener {
   public:
    void setConnectionHandler(const std::string& s,
                              const connection_handler& h) {
      _connectionAdress = s;
